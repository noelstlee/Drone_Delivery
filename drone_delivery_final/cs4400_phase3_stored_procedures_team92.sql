...
sp_main: begin
	...
	if ip_uname is null or ip_first_name is null or ip_last_name is null
       or ip_address is null or ip_rating is null
	   or ip_credit is null then leave sp_main;
	end if;
    
    if ip_rating < 0 or ip_credit < 0 then 
		leave sp_main; 
    end if;
       
		select count(*) into uname_exists from users where uname = ip_uname;

		if uname_exists = 0 and (ip_rating > 0) and (ip_rating < 6) then
...
-- parameter validation check
    if ip_uname is null or 
	   ip_first_name is null or 
	   ip_last_name is null or 
	   ip_address is null or 
	   ip_taxID is null or
	   ip_service is null or 
	   ip_salary is null or 
	   ip_licenseID is null or 
	   ip_experience is null then leave sp_main;
	end if;
    
    if ip_service < 0 or ip_salary < 0 or ip_experience < 0 then
		leave sp_main;
	end if;
...
sp_main: begin
	...
	if ip_storeID is null or
		ip_droneTag is null or
        ip_capacity is null or 
        ip_remaining_trips is null or 
		ip_pilot is null then leave sp_main;
	end if;
    
    if ip_droneTag < 0 or ip_capacity < 0 or ip_remaining_trips < 0 then
		leave sp_main;
	end if;
...
sp_main: begin
	if ip_uname is null or 
		ip_money is null then leave sp_main;
	end if;
...
sp_main: begin
    ...
    declare ip_drone_count int;
 	...
 	if ip_incoming_pilot is null or 
		ip_outgoing_pilot is null then leave sp_main;
	end if;
...
sp_main: begin
 	if ip_drone_store is null or 
		ip_drone_tag is null or
        ip_refueled_trips is null 
        then leave sp_main;
	end if;
    
    if ip_drone_tag < 0 or ip_refueled_trips < 0 then
		leave sp_main;
	end if;
...
sp_main: begin
	...
	if ip_orderID is null or
	   ip_sold_on is null or 
	   ip_purchased_by is null or 
	   ip_carrier_store is null or 
	   ip_carrier_tag is null or 
	   ip_barcode is null or 
	   ip_price is null or 
	   ip_quantity is null then leave sp_main;
	end if;
    
    if ip_carrier_tag < 0 or ip_price < 0 or ip_quantity < 0 then
		leave sp_main;
	end if;
...
sp_main: begin
    ...
    if ip_orderID is null then leave sp_main;
		...
		if v_remaining_trips > 0 then
			...
			update customers
			set rating = case 
							when (v_cost > 25 and rating < 5) then rating + 1
							else rating
						 end
...
sp_main: begin
	...
	if ip_barcode is null then leave sp_main;
    end if;
...
sp_main: begin
    ...
    if ip_storeID is null or ip_droneTag is null then leave sp_main;
    end if;
    
    if ip_droneTag < 0 then 
		leave sp_main;
    end if;
